...
/*   Updated: 2025/01/26 23:52:28 by mmalie           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	opt_rb(t_stk *stk_a, t_stk *stk_b, t_list *cmd_list, int tgt_i)
{
	int	top_i;
	int	next_highest;
	...
	if (tgt_i >= (top_i / 2))
	{
		if (r_and_pb(stk_a, stk_b, cmd_list, tgt_i) != 0)
			return (1);
	}
	else
	{
...
int	ps_to_a(t_stk *stk_a, t_stk *stk_b, t_list *cmd_list)
{
	...
	{
		find_highest(stk_b);
		if ((opt_rb(stk_a, stk_b, cmd_list, stk_b->highest_pos) != 0)
			|| (pusher(stk_a, stk_b, cmd_list, "pa") != 0))
			return (1);
		if ((stk_a->nb_elem > 1)
